...
class TestSequenceFeatures:
    ...
    def sample_dataset(self):
        ...
        metadata = pd.DataFrame(
            {
                "prediction": [
                    ["A", "C", "D", "K"],  # Tryptic (ends with K)
                    ["G", "H", "I", "R"],  # Tryptic (ends with R)
                    ["L", "M", "N"],  # Non-tryptic (ends with N)
                    ["P", "Q"],  # Non-tryptic (ends with Q)
                ],
    ...
    def test_compute_is_c_term_tryptic(self, sequence_features, sample_dataset):
        """Test that is_c_term_tryptic is computed correctly."""
        sequence_features.prepare(sample_dataset)
        sequence_features.compute(sample_dataset)

        assert "is_c_term_tryptic" in sample_dataset.metadata.columns
        # First two end with K/R (tryptic), last two don't
        expected_tryptic = [True, True, False, False]
        assert list(sample_dataset.metadata["is_c_term_tryptic"]) == expected_tryptic
