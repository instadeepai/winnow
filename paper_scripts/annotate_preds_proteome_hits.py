...
Paper full-search recompute uses this **after** ``winnow predict`` so deposited
``general_results/full/`` trees stay aligned (FDR on all PSMs, then drop short
peptides and add ``proteome_hit``). That differs from the package CLI
``winnow annotate-proteome-hits``, which annotates spectra + de novo preds into a
Winnow dataset directory *before* predict. See ``paper_scripts/README.md``.

Each project is a subfolder of ``--predictions-root`` containing
