...
def _load_metadata(
    ...
    for path in (metadata_train, metadata_val):
        if path is not None:
    ...
    logger.info(
        "Combined metadata: %s rows, %s columns",
        f"{len(df_meta):,}",
        len(df_meta.columns),
    )
...
@app.command()
def main(
    features_train: Annotated[
        Path,
        typer.Option(
            "--features-train",
            help=(
                "Path to the training features parquet produced by "
                "`winnow compute-features`."
            ),
        ),
    ],
    features_val: Annotated[
        Optional[Path],
        typer.Option(
            "--features-val",
            help=(
    ...
    metadata_train: Annotated[
        Optional[Path],
        typer.Option(
            "--metadata-train",
            help=(
                "Optional path to full training metadata parquet (produced by "
                "compute-features with metadata_output_path set).  Enables mirror "
                "plots, RT scatter, and token stems."
            ),
        ),
    ] = None,
    metadata_val: Annotated[
        Optional[Path],
        typer.Option(
            "--metadata-val",
    ...
    predictions_csv: Annotated[
        ...
        typer.Option(
            "--predictions-csv",
            help=(
                "Optional path to InstaNovo-style predictions CSV with beam columns.  "
    ...
    plots_dir: Annotated[
        ...
        typer.Option(
            "--plots-dir",
            help=(
                "Directory for output plots. Defaults to a "
                "`feature_investigation_plots` subdirectory next to --features-train."
            ),
        ),
    ] = None,
) -> None:
    ...
    logger.info("Loading training features from %s", features_train)
    df = pl.read_parquet(features_train).to_pandas()
    ...
    df_meta = _load_metadata(metadata_train, metadata_val)

    has_metadata = df_meta is not None
    has_predictions_csv = predictions_csv is not None

    if plots_dir is None:
        resolved_plots = features_train.parent / "feature_investigation_plots"
    else:
        resolved_plots = plots_dir
    resolved_plots.mkdir(parents=True, exist_ok=True)
    logger.info("Saving plots to %s/", resolved_plots)

    logger.info("Dataset shape: %s", df.shape)
    n_correct = df["correct"].sum()
    n_incorrect = (~df["correct"]).sum()
    logger.info(
        "Correct: %s | Incorrect: %s | Total: %s",
        f"{n_correct:,}",
        f"{n_incorrect:,}",
        f"{len(df):,}",
    )
    logger.info("Class balance: %.1f%% correct", df["correct"].mean() * 100)

    n_steps = 7 + has_metadata * 2 + has_predictions_csv
    step = 0

    step += 1
    logger.info("[%s/%s] Confidence distribution...", step, n_steps)
    plot_confidence(df, resolved_plots)

    step += 1
    logger.info("[%s/%s] Mass error vs confidence...", step, n_steps)
    plot_mass_error(df, resolved_plots)

    if has_metadata:
        step += 1
        logger.info("[%s/%s] Mirror spectrum plots...", step, n_steps)
        plot_mirror_spectra(df_meta, resolved_plots)

    step += 1
    logger.info("[%s/%s] Fragment ion match features...", step, n_steps)
    plot_fragment_features(df, resolved_plots)

    step += 1
    logger.info("[%s/%s] iRT error...", step, n_steps)
    plot_irt(df, df_meta, resolved_plots)

    if has_metadata:
        step += 1
        logger.info("[%s/%s] Token-level stem plots...", step, n_steps)
        plot_token_stems(df_meta, resolved_plots)

    if has_predictions_csv:
        step += 1
        logger.info("[%s/%s] Beam confidence stem plots...", step, n_steps)
        assert predictions_csv is not None
        plot_beam_stems(predictions_csv, resolved_plots)

    step += 1
    logger.info("[%s/%s] Beam search features...", step, n_steps)
    plot_beam_features(df, resolved_plots)

    step += 1
    logger.info("[%s/%s] Token-level features...", step, n_steps)
    plot_token_features(df, resolved_plots)

    step += 1
    logger.info(
        "[%s/%s] Discriminative analysis (AUROC, correlation, violins, pairplot)...",
        step,
        n_steps,
    )
    plot_discriminative_analysis(df, resolved_plots)

    print_summary(df)

    logger.info("Done — plots saved to %s/", resolved_plots)
