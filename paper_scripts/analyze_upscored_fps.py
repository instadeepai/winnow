...
def main(
    ...
    if all_summary:
        ...
        combined.to_csv(results_dir / "upscored_summary.csv", index=False)
        logger.info("Summary table:\n%s", combined.to_string(index=False))

        with open(results_dir / "upscored_summary.json", "w") as f:
            json.dump(combined.to_dict(orient="records"), f, indent=2)

    if all_detail:
        detail_df = pd.concat(all_detail, ignore_index=True)
        detail_df.to_csv(results_dir / "upscored_fp_detail.csv", index=False)
        logger.info("FP detail table:\n%s", detail_df.to_string(index=False))

    logger.info(
        "Up-scored FP analysis complete. Tables in %s; plots in %s",
        results_dir,
        plots_dir,
    )
