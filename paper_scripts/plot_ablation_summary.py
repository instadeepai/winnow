...
The bar charts use ECE in the top 10% of PSMs ranked by calibrated
score. Tabular metrics use tail ECEat 5% and 10% FDR.
"""
...
def compute_tail_ece(
    pred: np.ndarray,
    labels: np.ndarray,
    tail_fraction: float = TAIL_ECE_FRACTION,
    n_bins: int = 10,
) -> float:
    """ECE in the top score-ranked fraction of PSMs."""
...
def metrics_from_eval_parquet(path: Path) -> dict[str, float | str]:
    """Compute top-decile/FDR-tail ECE and metrics from one eval-results Parquet."""
    ...
    pred = meta["calibrated_confidence"].to_numpy(dtype=float)
    labels = meta["correct"].to_numpy(dtype=float)
    tail_ece = compute_tail_ece(pred, labels)
    tail_ece_at_fdr = compute_tail_ece_at_fdr_thresholds(meta)
    ...
    return {
        ...
        "tail_ECE": round(tail_ece, 5),
        TAIL_ECE_COLUMN_BY_THRESHOLD[0.05]: round(tail_ece_at_fdr[0.05], 5),
        TAIL_ECE_COLUMN_BY_THRESHOLD[0.10]: round(tail_ece_at_fdr[0.10], 5),
...
def main(
    ...
    plots_dir: Annotated[
        Path,
        typer.Option("--plots-dir", help="Directory for figure outputs"),
    ] = _DEFAULT_OUTPUT_DIR,
    ...
    out_base = plots_dir / f"ablation_{metric_slug}_{slug}"
