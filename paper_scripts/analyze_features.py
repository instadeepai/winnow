...
def replot_from_pickles(
    results_dir: Path,
    output_dir: Path,
    *,
    correct_class_idx: int = 1,
) -> None:
    """Regenerate plots that only need deposited ``*.pkl`` artefacts.

    Loads ``perm_importance.pkl`` and ``shap_values.pkl`` from ``results_dir``
    (e.g. Figshare ``feature_importance/PXD014877/``). Writes permutation
    importance, SHAP summary / dependence / interactions / heatmap under
    ``output_dir``.

    Skips the SHAP bar plot (needs scaled test features for clustering) and
    the correlation matrix (needs the training feature matrix); neither is in
    the Figshare deposit.
    """
    perm_path = results_dir / "perm_importance.pkl"
    shap_path = results_dir / "shap_values.pkl"
    missing = [str(p) for p in (perm_path, shap_path) if not p.is_file()]
    if missing:
        raise FileNotFoundError(
            "Replot requires both pickles under "
            f"{results_dir}: missing {', '.join(missing)}"
        )

    output_dir.mkdir(parents=True, exist_ok=True)

    logger.info("Loading permutation importance from %s", perm_path)
    with open(perm_path, "rb") as handle:
        perm_importance = pickle.load(handle)

    logger.info("Loading SHAP values from %s", shap_path)
    with open(shap_path, "rb") as handle:
        shap_values = pickle.load(handle)

    display_names = list(shap_values.feature_names)
    if len(display_names) != len(perm_importance.importances_mean):
        raise ValueError(
            "Feature count mismatch between shap_values.feature_names "
            f"({len(display_names)}) and perm_importance.importances_mean "
            f"({len(perm_importance.importances_mean)})"
        )

    perm_importance_dict = dict(zip(display_names, perm_importance.importances_mean))

    logger.info("Replotting from pickles (skipping SHAP bar and feature correlations)")
    plot_feature_importance(
        perm_importance_dict,
        "Permutation feature importance",
        output_dir / "permutation_importance",
    ...
    plot_shap_summary(shap_values, correct_class_idx, output_dir)
    plot_shap_dependence(
        shap_values, display_names, display_names, correct_class_idx, output_dir
    )
    plot_shap_interactions(
        shap_values, display_names, display_names, correct_class_idx, output_dir
    )
    plot_shap_heatmap(shap_values, correct_class_idx, output_dir)
    logger.info("Replot complete; plots written to %s", output_dir)
...
def main(
    ...
    model_path: Annotated[
        ...
        typer.Option(
            help=(
                "Path to pretrained calibrator model directory "
                "(required unless --replot-dir is set)."
            ),
        ),
    ] = None,
    replot_dir: Annotated[
        Optional[Path],
        typer.Option(
            "--replot-dir",
            help=(
                "Directory with deposited perm_importance.pkl and shap_values.pkl "
                "(e.g. paper_data/feature_importance/PXD014877). Skips recomputation "
                "and regenerates plots that do not need feature matrices."
            ),
        ),
    ] = None,
    data_dir: Annotated[
    ...
    if replot_dir is not None:
        replot_from_pickles(replot_dir, output_dir)
        return

    if model_path is None:
        raise typer.BadParameter("--model-path is required unless --replot-dir is set")
