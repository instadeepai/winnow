...
- Glissade is installed from the ``paper`` dependency group
  (``uv sync --group paper``), which pins
  ``https://github.com/JemmaLDaniel/glissade.git`` branch ``winnow-benchmark``
  at ``7c723a2af4a88fda84a6bd4f223b351179bd36da``.
"""
...
def _load_glissade_functions():
    """Import Glissade FDR helpers from the installed ``paper`` dependency group."""
    try:
        module = importlib.import_module("glissade.glissade")
    except ImportError as exc:
        raise ImportError(
            "Glissade is required for this benchmark. Install it with "
            "`uv sync --group paper` (pins JemmaLDaniel/glissade @ "
...
def main(
    ...
    plots_dir: Annotated[
        ...
        typer.Option(
            "--plots-dir",
            help="Directory for png/pdf figures (required when plotting).",
        ),
    ] = None,
    datasets: Annotated[
        Optional[list[str]],
    ...
    if plot:
        ...
        if plots_dir is not None:
            raise typer.BadParameter(
                "Do not pass --plots-dir with --no-plot; this run writes tables only."
            )
        results_dir.mkdir(parents=True, exist_ok=True)

    if summarise_only is not None:
        results = pd.read_csv(summarise_only)
        write_holdout_summary_tables(results, results_dir)
        if plot:
            assert plots_dir is not None
            plot_benchmark_results(results, plots_dir)
        return
    ...
    results_path = results_dir / "external_peptide_holdout_results.csv"
