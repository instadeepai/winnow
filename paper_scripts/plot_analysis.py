...
Produces calibration, PR, confidence, FDR-accuracy and labelled-diagnostic
figures from a predict directory, annotated against a FASTA proteome.
"""
...
# Features used by the paper HeLa InstantNovo / Casanovo calibrators (and the
# general model). Prefer ``config.json`` ``feature_columns`` via ``--model-dir``.
_DEFAULT_TRAINED_FEATURE_COLUMNS = [
    "confidence",
    "ion_matches",
    "ion_match_intensity",
    "irt_error",
...
def _resolve_pca_feature_columns(model_dir: Path | None) -> list[str]:
    """Return MLP input columns: confidence + checkpoint ``feature_columns``."""
    if model_dir is None:
        return list(_DEFAULT_TRAINED_FEATURE_COLUMNS)
    config_path = model_dir / "config.json"
    if not config_path.is_file():
        logger.warning(
            "No config.json under %s; using default trained feature columns for PCA",
            model_dir,
        )
        return list(_DEFAULT_TRAINED_FEATURE_COLUMNS)
    ...
    if not isinstance(feature_columns, list) or not feature_columns:
        logger.warning(
            "config.json under %s has no feature_columns; using default PCA set",
            model_dir,
        )
        return list(_DEFAULT_TRAINED_FEATURE_COLUMNS)
    return ["confidence", *[str(c) for c in feature_columns]]
...
def plot_pca_features(
    ...
    """PCA of trained calibrator features coloured by correctness."""
    if feature_cols is None:
        feature_cols = list(_DEFAULT_TRAINED_FEATURE_COLUMNS)
    available = [c for c in feature_cols if c in df.columns]
    missing = [c for c in feature_cols if c not in df.columns]
    if missing:
        logger.warning("PCA skipped missing trained feature columns: %s", missing)
    if len(available) < 2:
        raise ValueError(
            f"Need at least 2 trained feature columns for PCA; found {available}"
        )
...
def main(
    ...
    split: Annotated[
        ...
        typer.Option(
            "--split",
            help=(
                "Split id for filenames; titles use test set / unlabelled space / "
                "full search space"
            ),
        ),
    ],
    label_mode: Annotated[
        Literal["labelled", "unlabelled"],
        typer.Option(
            "--label-mode",
    ...
    fasta: Annotated[
        Path,
        typer.Option("--fasta", help="FASTA file for proteome annotation"),
    ],
    plots_dir: Annotated[
        Optional[Path],
        typer.Option(
            "--plots-dir",
            help="Directory for plots (defaults to predictions-dir/plots/).",
        ),
    ] = None,
    model_dir: Annotated[
        Optional[Path],
        typer.Option(
            "--model-dir",
            help=(
                "Calibrator directory (config.json + optional training_history). "
                "PCA uses checkpoint feature_columns (+ confidence)."
            ),
        ),
    ] = None,
    dns_model: Annotated[
        ...
        typer.Option(
            "--dns-model",
            help=(
                "Upstream DNS model name for plot titles "
                "(e.g. InstaNovo, Casanovo, $\\pi$-PrimeNovo)"
            ),
        ),
    ] = None,
) -> None:
    ...
    out_dir = plots_dir if plots_dir is not None else predictions_dir / "plots"
    ...
    if labelled:
        ...
        split_label = _split_display(split)
        print("Plotting ranked q-value (proteome mapping)")
        fig = plot_ranked_qvalue(
            df,
            "proteome_hit",
            residue_masses,
        ...
        _save(fig, out_dir, f"ranked_qvalue_{split}_proteome")
