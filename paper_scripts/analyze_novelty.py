...
def _nontryptic_digest_analysis(
    ...
    *,
    ...
    summary.to_csv(results_dir / f"{file_prefix}_tryptic_summary.csv", index=False)
    print(summary.to_string(index=False))

    print("Building terminus proportion table")
    prop_df = _nontryptic_terminus_proportions_table(df)
    prop_df.to_csv(results_dir / f"{file_prefix}_terminus_proportions.csv", index=False)
    ...
    if "confidence" in df.columns:
        ...
        delta_df.to_csv(
            results_dir / f"{file_prefix}_calibration_delta_summary.csv", index=False
        )
    ...
    if not feat_df.empty:
        feat_df.to_csv(
            results_dir / f"{file_prefix}_feature_comparison.csv", index=False
        )

    plot_kw = {"prefix": file_prefix, "dataset_label": dataset_label}
    print("Plotting")
    _plot_nontryptic_conf_by_terminus(df, plots_dir, **plot_kw)
    _plot_nontryptic_raw_conf_by_terminus(df, plots_dir, **plot_kw)
    _plot_nontryptic_score_histograms_at_fdr(df, plots_dir, **plot_kw)
    _plot_nontryptic_raw_score_histograms_at_fdr(df, plots_dir, **plot_kw)
    _plot_nontryptic_full_score_histograms(df, plots_dir, **plot_kw)
    _plot_nontryptic_calibration_scatter(df, plots_dir, **plot_kw)
    _plot_nontryptic_delta_by_terminus(df, plots_dir, **plot_kw)
    _plot_nontryptic_feature_comparison(feat_df, plots_dir, **plot_kw)

    print(
        f"\n{dataset_label} analysis complete. "
        f"Tables in {results_dir}; plots in {plots_dir}"
    )
...
def nontryptic_digest(
    ...
    results_dir: Annotated[
        Path,
        typer.Option("--results-dir", help="Directory for CSV tables."),
    ],
    plots_dir: Annotated[
        Path,
...
def proteometools(
    ...
    summary.to_csv(results_dir / "proteometools_novelty_summary.csv", index=False)
    print(summary.to_string(index=False))
    ...
    if not feat_df.empty:
        feat_df.to_csv(
            results_dir / "proteometools_feature_comparison.csv", index=False
        )

    print("Plotting")
    _plot_proteometools_conf_by_category(df, plots_dir)
    _plot_proteometools_hit_rate(df, plots_dir)
    _plot_proteometools_feature_comparison(feat_df, plots_dir)

    print(
        f"\nProteomeTools-1 analysis complete. "
        f"Tables in {results_dir}; plots in {plots_dir}"
    )
