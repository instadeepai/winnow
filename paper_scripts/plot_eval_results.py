...
def _safe_stem(key: str) -> str:
    """Return a flat, filesystem-safe filename stem for a project key."""
...
def plot_precision_recall(
    ...
    _style_ax(ax)
    fig.tight_layout()
    _save_fig(fig, output_dir / f"pr_curve_{_safe_stem(project)}")
...
def plot_fdr_run(
    ...
    _style_ax(ax)
    fig.tight_layout()
    _save_fig(fig, output_dir / f"fdr_run_{_safe_stem(project)}")
...
def plot_q_value_run(
    ...
    _style_ax(ax)
    fig.tight_layout()
    _save_fig(fig, output_dir / f"qvalue_run_{_safe_stem(project)}")
...
def plot_fdr_run_with_bands(
    ...
    _style_ax(ax)
    fig.tight_layout()
    _save_fig(fig, output_dir / f"fdr_run_bands_{_safe_stem(project)}")
...
def plot_q_value_run_with_bands(
    ...
    _style_ax(ax)
    fig.tight_layout()
    _save_fig(fig, output_dir / f"qvalue_run_bands_{_safe_stem(project)}")
...
def plot_true_vs_estimated_fdr(
    ...
    *,
    ...
    _save_fig(fig, output_dir / f"{tag}_{_safe_stem(project)}")
...
def plot_calibration(
    ...
    _style_ax(ax)
    fig.tight_layout()
    _save_fig(fig, output_dir / f"calibration_{_safe_stem(project)}")
...
def plot_score_histograms(
    ...
    fig.tight_layout()
    _save_fig(fig, output_dir / f"score_histograms_{_safe_stem(project)}")
...
def main(
    ...
    results_dir: Annotated[
        Path,
        typer.Option("--results-dir", help="Directory for summary/diagnostics CSVs."),
    ],
    plots_dir: Annotated[
        Path,
    ...
    for project in project_list:
        ...
        df[summary_cols].to_csv(results_dir / f"{stem}_summary.csv", index=False)

        diag = _compute_diagnostics(df, eval_type)
        diag.to_csv(results_dir / f"{stem}_diagnostics.csv", index=False)
        logger.info("  Diagnostics saved (%d alpha levels)", len(diag))

        generate_all_plots(df, project, eval_type, plots_dir)
        logger.info("  Plots saved to %s", plots_dir)

    logger.info("Done. Tables in %s; plots in %s", results_dir, plots_dir)
