...
def process_dataset(cfg: DatasetConfig, output_dir: Path) -> pd.DataFrame:
    ...
    plot_dataset_from_curves(curves, cfg.key, output_dir)
    return curves
...
def main(
    ...
    if summarise_only is not None:
        ...
        for key in dataset_keys:
            ...
            plot_dataset_from_curves(curves, str(key), output_dir)
        ...
        write_summary_tables(
            acceptance, error_gain, output_dir, "fdr_method_comparison"
        )
        if summarise_only.resolve() != (output_dir / CURVES_CSV_NAME).resolve():
            write_curves_csv(curves_out, output_dir)
        return
    ...
    for key in dataset_keys:
        ...
        curve_parts.append(process_dataset(configs[key], output_dir))

    curves = pd.concat(curve_parts, ignore_index=True)
    write_curves_csv(curves, output_dir)
