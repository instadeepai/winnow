...
class ChimericFeatures(CalibrationFeatures):
    ...
    def compute(self, dataset: CalibrationDataset) -> None:
        ...
        dataset.metadata["runner_up_prosit_mz"] = predictions_per_peptide["mz"].reindex(
            dataset.metadata["spectrum_id"], fill_value=np.nan
        )
        dataset.metadata["runner_up_prosit_intensity"] = predictions_per_peptide[
            "intensities"
        ...
        dataset.metadata["chimeric_xcorr"] = xcorr
