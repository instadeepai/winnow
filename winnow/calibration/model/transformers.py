...
class ConditionalTransformerEncoderBlock(nnx.Module):
    """Conditional Transformer encoder block with self-attention, conditional attention, and feedforward layers."""
    ...
    def __init__(
        ...
        self.conditional_attn_layernorm = nnx.LayerNorm(
            num_features=dim_model, rngs=rngs
        )
    ...
    def __call__(
        ...
        if input_mask.ndim == 2:
            ...
            ]  # originally (batch_size, seq_len), now (batch_size, num_self_heads, seq_len, seq_len)
        if condition_mask.ndim == 2:
            condition_mask = condition_mask[
                :, jnp.newaxis, jnp.newaxis, :
            ]  # originally (batch_size, cond_seq_len), now (batch_size, num_condition_heads, seq_len, cond_seq_len)
