...
class ModifiedResidueEncoder(nnx.Module):
    ...
    def __init__(
        self, num_residues: int, num_modifications: int, dim_residue: int,
        dim_modification: int, dim_model: int, rngs: nnx.Rngs
    ):
        ...
        self.modification_embedding = nnx.Embed(num_modifications, dim_modification, rngs=rngs)

        # Initialize projection layer
        self.projection = nnx.Linear(dim_residue + dim_modification, dim_model, rngs=rngs)

    def __call__(self, residue_indices: jnp.ndarray, modification_indices: jnp.ndarray) -> jnp.ndarray:
...
class ConditionalPeptideEncoder(nnx.Module):
    def __init__(
        self, config: PeptideEncoderConfig, rngs: nnx.Rngs,
    ):
        self.config = config
    ...
    def __call__(
        self, residue_ids: jnp.ndarray, modification_ids: jnp.ndarray,
        input_mask: jnp.ndarray, condition_embedding: jnp.ndarray,
        ...
        cls_embeddings = jnp.repeat(
            self.cls_embedding[jnp.newaxis, jnp.newaxis, :], repeats=residue_ids.shape[0], axis=0
        )
        ...
        input_mask = jnp.concatenate(
            [jnp.full((input_mask.shape[0], 1), True, dtype=input_mask.dtype), input_mask], axis=1
        )
