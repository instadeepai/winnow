...
@app.command(
    ...
    help=(
        "Compute calibration features and write enriched metadata "
        "(and optionally a lean training matrix).\n\n"
        "Loads the dataset using the same options as training, instantiates the "
        "calibrator feature stack from config, always runs "
        "``calibrator.compute_features``, "
        "and saves outputs without fitting the network or saving a model. "
        "When ``labelled=true``, spectrum "
        "data must include a ``sequence`` column.\n\n"
        "[bold cyan]Quick start:[/bold cyan]\n"
        "  [dim]winnow compute-features[/dim]  # Uses configs/compute_features.yaml\n\n"
        "[bold cyan]Override parameters:[/bold cyan]\n"
        "  [dim]winnow compute-features metadata_output_path=results/metadata.csv[/dim]\n"
        "  [dim]winnow compute-features training_matrix_output_path=results/features.parquet[/dim]\n"
        "  [dim]winnow compute-features labelled=false[/dim]\n\n"
        "[bold cyan]Custom config directory:[/bold cyan]\n"
...
@config_app.command(
    ...
    help=(
        ...
        "  [dim]winnow config compute-features metadata_output_path=out.csv[/dim]\n"
