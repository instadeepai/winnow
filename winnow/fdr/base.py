...
class FDRControl(metaclass=ABCMeta):
    ...
    def compute_fdr(self, score: float) -> float:
        ...
        if idx == len(self._confidence_scores) and score < self._confidence_scores[-1]:
            warnings.warn(
                f"Score {score} is below the range of fitted confidence scores (min: {self._confidence_scores[-1]}). "
                f"Cannot compute FDR from fitted data. Returning conservative FDR estimate of 1.0.",
                UserWarning,
            )
            return 1.0
        # If the score is above the range of fitted confidence scores, return a conservative estimate of the FDR at the lowest score
            ...
            warnings.warn(
                f"Score {score} is above the range of fitted confidence scores (max: {self._confidence_scores[0]}). "
                f"Cannot compute FDR from fitted data. Returning conservative FDR estimate of {self._fdr_values[0]}.",
                UserWarning,
            )
